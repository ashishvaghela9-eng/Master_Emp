...
import { Plus, Trash2, ChevronRight, Server, Shield, Database, Loader2, Info, X, ArrowUp, ArrowDown, Table2 } from "lucide-react";
...
function FieldRow({ field, isBuiltIn, onDelete, onMoveUp, onMoveDown, isFirst, isLast }: { field: ServiceField; isBuiltIn: boolean; onDelete: () => void; onMoveUp: () => void; onMoveDown: () => void; isFirst: boolean; isLast: boolean; }) {
...
function ServicePanel({ svc, onDelete }: { svc: ServiceDefinition; onDelete: () => void }) {
  const [showAddField, setShowAddField] = useState(false);
  const qc = useQueryClient();
  const { toast } = useToast();
  const deleteFieldMutation = useMutation({
    mutationFn: ({ fieldId }: { fieldId: number }) => deleteField(svc.id, fieldId),
  ...
  const handleMoveField = (fields: ServiceField[], fromIdx: number, toIdx: number) => {
  ...
  return (
    <div className="bg-card rounded-xl border h-full flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b">
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => setShowAddField(p => !p)}><Plus className="w-3.5 h-3.5 mr-1" /> Add Field</Button>
          {!svc.isBuiltIn && (
            <Button size="sm" variant="outline" className="h-8 text-xs text-rose-600 border-rose-200 hover:bg-rose-50" onClick={onDelete}><Trash2 className="w-3.5 h-3.5 mr-1" /> Delete</Button>
          )}
        </div>
      </div>
      <div className="px-5 py-3 border-b bg-muted/10 flex items-center gap-2 text-xs text-muted-foreground">
        <Database className="w-3.5 h-3.5 text-primary" />
        <span>Slug: <code className="font-mono bg-muted px-1 py-0.5 rounded">{svc.slug}</code></span>
        ...
        {svc.fields.length === 0 ? (
          ...
          <>
            ...
            {svc.fields.map((field, idx) => (
              <FieldRow key={field.id} field={field} isBuiltIn={svc.isBuiltIn} isFirst={idx === 0} isLast={idx === svc.fields.length - 1}
                onDelete={() => deleteFieldMutation.mutate({ fieldId: field.id })}
                onMoveUp={() => handleMoveField(svc.fields, idx, idx - 1)}
                onMoveDown={() => handleMoveField(svc.fields, idx, idx + 1)}
              />
            ))}
          </>
        )}
...
function RecordsPanel({ svc }: { svc: ServiceDefinition }) {
  ...
  const fields = data?.definition?.fields ?? svc.fields;
  const records: ServiceRecord[] = data?.records ?? [];

  const addMutation = useMutation({
    mutationFn: () => addRecord(svc.slug, newRecord),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/services", svc.slug] });
      toast({ title: "Record added", description: `New entry added to ${svc.name}.` });
      setAddOpen(false);
      setNewRecord({});
    },
    onError: (e: any) => toast({ title: "Failed", description: e.message, variant: "destructive" }),
  ...
  const displayFields = fields.filter(f => f.fieldName !== "id" && f.fieldName !== "created_at" && f.fieldName !== "updated_at");
  ...
  return (
    <div className="bg-card rounded-xl border h-full flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b">
        ...
        <Button size="sm" className="h-8 text-xs" onClick={() => { setNewRecord({}); setAddOpen(true); }}>
      ...
      <div className="px-5 py-3 border-b bg-muted/10 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span className="text-border">·</span>
        <span>{displayFields.length} columns</span>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4">
        {isLoading ? (
          ...
          <div className="text-center py-12 border border-dashed rounded-xl text-muted-foreground text-sm">
            No fields defined for this service. Go to "Add Services" → field management to add columns.
          </div>
        ) : records.length === 0 ? (
          <div className="text-center py-12 border border-dashed rounded-xl text-muted-foreground text-sm">
            No entries yet. Click "Add Entry" to create the first record.
          </div>
        ) : (
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-muted/50">
                  ...
                  {displayFields.map(f => (
                    <th key={f.id} className="text-left px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wide border-b whitespace-nowrap">{f.fieldLabel}</th>
                  ))}
              ...
              <tbody>
                {records.map((rec, idx) => (
                  <tr key={rec.id} className="hover:bg-muted/20 border-b last:border-b-0">
                    ...
                    {displayFields.map(f => (
                      <td key={f.id} className="px-3 py-2 max-w-[200px] truncate" title={String(rec[f.fieldName] ?? "")}>
                        {rec[f.fieldName] != null ? String(rec[f.fieldName]) : <span className="text-muted-foreground/50">—</span>}
                      </td>
                    ))}
                    <td className="px-3 py-2 text-right">
                      {deleteConfirmId === rec.id ? (
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-rose-500 hover:bg-rose-50" onClick={() => setDeleteConfirmId(rec.id)}>
                          <Trash2 className="w-3.5 h-3.5" />
                        </Button>
                      )}
                    </td>
      ...
      <Dialog open={addOpen} onOpenChange={v => { if (!v) { setAddOpen(false); setNewRecord({}); } }}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Plus className="w-5 h-5 text-primary" /> Add Entry — {svc.name}</DialogTitle>
          </DialogHeader>
            ...
            {displayFields.length === 0 ? (
              ...
              <div key={f.id} className="space-y-1.5">
                ...
                <Input
                  ...
                  value={newRecord[f.fieldName] ?? ""}
                  onChange={e => setNewRecord(p => ({ ...p, [f.fieldName]: e.target.value }))}
          ...
          <DialogFooter>
            ...
            <Button onClick={() => addMutation.mutate()} disabled={addMutation.isPending}>
              {addMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Plus className="w-4 h-4 mr-2" />} Add Entry
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export default function ServicesConfig() {
  ...
  const tableServices = services.filter(s => s.hasTable || s.isBuiltIn);
    ...
    <AppLayout title="Add Services">
      <div className="space-y-4 h-full" style={{ minHeight: "calc(100vh - 130px)" }}>
        ...
        <div className="flex gap-1 bg-muted/40 p-1 rounded-xl w-fit border">
          ...
          >
            <Table2 className="w-4 h-4 inline mr-2 -mt-0.5" />
          ...
          <div className="w-72 flex-shrink-0 flex flex-col">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-sm text-foreground">
                {activeTab === "standalone" ? "Standalone Services" : "Table-Backed Services"}
              </h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-1.5 bg-card border rounded-xl p-2">
              {isLoading ? (
                ...
                <div className="text-center py-8 text-sm text-muted-foreground px-3">
                  {activeTab === "standalone" ? "No standalone services yet. Click 'New' to create one." : "No table-backed services yet."}
                </div>
              ) : currentList.map(svc => (
                <button
                  key={svc.id}
            ...
            {!selectedSvc ? (
              <div className="flex flex-col items-center justify-center h-full text-center gap-3 bg-card rounded-xl border p-10">
                ...
                <p className="text-muted-foreground font-medium">
                  {activeTab === "standalone" ? "Select a service to manage its fields" : "Select a service to view and manage its records"}
                </p>
                <p className="text-xs text-muted-foreground">Or create a new service using the button on the left</p>
              </div>
            ) : activeTab === "standalone" ? (
              <ServicePanel svc={selectedSvc} onDelete={() => setDeleteConfirm(selectedSvc)} />
            ) : (
              <RecordsPanel svc={selectedSvc} />
            )}
          </div>
        </div>
      </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {createTable ? "A new database table will be created with the fields you define." : "Add a tracked service without creating a database table."}
            </p>
          </DialogHeader>
          ...
          <div className="space-y-5 py-2">
            <div className="space-y-1.5">
              ...
              {newSvcName && (
                <p className="text-xs text-muted-foreground">
                  {createTable ? "Table name" : "Slug"}: <code className="font-mono bg-muted px-1 rounded">
                    {createTable ? "svc_" : "svc_noTable_"}{newSvcName.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_")}
                  </code>
                </p>
              )}
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label>Fields {createTable ? "(define table columns)" : "(optional)"}</Label>
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            >
              ...
              {createTable ? "Create Service & Table" : "Create Service"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={v => { if (!v) setDeleteConfirm(null); }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-rose-600">Delete Service</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
